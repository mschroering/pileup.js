...
var PileupCache = (function () {
  ...
  _createClass(PileupCache, [{ key: 'groupKey', value: function groupKey(read) {if (this.viewAsPairs) {return read.name + ':' + read.ref;} else {return read.getKey();}} // Load a new read into the visualization cache.
    ...
    // maximum read depth. This is 'chr'-agnostic.
      ...
      return _underscore2['default'].filter(this.groups, function (group) {return group.span.intersects(range);});} // Determine the number of groups at a locus.
    // Like getGroupsOverlapping(range).length > 0, but more efficient.
  }, { key: 'anyGroupsOverlapping', value: function anyGroupsOverlapping(range) {for (var k in this.groups) {var group = this.groups[k];if (group.span.intersects(range)) {return true;}}return false;} // Re-sort the pileup so that reads overlapping the locus are on top.
      ...
      var groups = _underscore2['default'].filter(this.groups, function (group) {return _underscore2['default'].any(group.alignments, function (a) {return a.read.getInterval().containsLocus(contig, position);});}); // For each row, find the left-most point (for sorting).
